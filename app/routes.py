...
async def nearest_gp_pharmacy(latitude: float, longitude: float):
    ...
    if not (-90 <= latitude <= 90) or not (-180 <= longitude <= 180):
        ...
        return JSONResponse(
            status_code=400,
            content={"error": "Invalid latitude or longitude."}
        )
    ...
    if gp_gdf is None or gp_gdf.empty:
        logger.error(
            "GP GeoJSON data is not available."
        )
        return JSONResponse(
            content={"error": "GP data not available"},
            status_code=500
        )

    if pharmacy_gdf is None or pharmacy_gdf.empty:
        logger.error(
            "Pharmacy GeoJSON data is not available."
        )
        return JSONResponse(
            content={"error": "Pharmacy data not available"},
            status_code=500
        )
    ...
    nearest_gp_point = gpd.GeoSeries([Point(nearest_gp['geometry'].x,
                                            nearest_gp['geometry'].y)])
    ...
    nearest_pharmacy_to_gp = pharmacy_gdf_bng.loc[
        pharmacy_gdf_bng['distance'].idxmin()
    ]
...
async def find_bins():
    ...
    if bins_gdf is None or bins_gdf.empty:
        logger.error(
            "Dog or Litter Bin GeoJSON data is not available."
        )
        return JSONResponse(
            content={"error": "Dog or Litter Bin data not available"},
            status_code=500
        )

    if nature_reserves_gdf is None or nature_reserves_gdf.empty:
        logger.error(
            "Nature Reserve GeoJSON data is not available."
        )
        return JSONResponse(
            content={"error": "Nature Reserve data not available"},
            status_code=500
        )

    if conservation_areas_gdf is None or conservation_areas_gdf.empty:
        logger.error(
            "Conservation Area GeoJSON data is not available."
        )
        return JSONResponse(
            content={"error": "Conservation Area data not available"},
            status_code=500
        )

    # Get number of bins (points) within each nature reserve (polygons)
    bins_in_nature_reserves = gpd.sjoin(bins_gdf, nature_reserves_gdf,
                                        how="inner", predicate="within")
    # Get number of bins (points) within each conservation area (polygons)
    bins_in_conservation_areas = gpd.sjoin(bins_gdf, conservation_areas_gdf,
                                           how="inner", predicate="within")
...
async def upload_geojson(file: UploadFile = File(...)):
    ...
    try:
        if not file.filename.endswith(".geojson"):
            logger.warning(
                f"Uploaded file is not of type GeoJSON: {file.content_type}"
            )
        ...
        if "features" not in geojson_data:
            logger.warning(
                "Uploaded GeoJSON file does not contain features: "
                f"{file.filename}"
            )
